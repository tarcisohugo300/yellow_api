...
module.exports.controller = (apiRouter, _io, _socket_list) => {
                ...
                if (result.length > 0) {
                    ...
                    db.query("UPDATE `user_detail` SET `auth_token` = ?, `modify_date` = NOW() WHERE `mobile` = ? AND  `mobile_code` = ? AND `user_type` = ? ", [auth_token, reqObj.mobile, reqObj.mobile_code, reqObj.user_type], (err, uResult) => {
                        ...
                        if (uResult.affectedRows > 0) {
                            getUserDetailUserId(result[0].user_id, (_isDone, uObj) => {
                    ...
                    db.query("INSERT INTO `user_detail` (  `mobile`, `mobile_code`,`user_type`, `push_token`, `auth_token`, `device_source`, `status`  ) VALUES (?,?,?, ?,?,?, ?)", [
                        ...
                        if (result) {
                            getUserDetailUserId(result.insertId, (_isDone, uObj) => {
                                res.json({ "status": "1", "payload": uObj })
                            })
                        } else {
                        ...
                        if (result.length > 0) {
                            ...
                            if (reqObj.is_online == 0) {
                                ...
                                if (result[0].status == 0 || result[0].status == -1) {
                                    ...
                                    res.json({
                                        "status": "0",
                                        "message": "Your account not apiRouterroved"
                                    })
                                    return
                                }
                ...
                db.query("SELECT `user_id`, `mobile`, `mobile_code` FROM `user_detail` WHERE `user_id` != ? AND `mobile` = ? AND `mobile_code` = ?   ", [uObj.user_id, reqObj.mobile, reqObj.mobile_code], (err, result) => {
                    ...
                    if (result.length == 0) {
                        ...
                        db.query("UPDATE  `user_detail` SET `name` = ?, `email` = ?, `gender` = ?, `mobile` = ? , `mobile_code` = ?, `zone_id` = ?, `select_service_id` = ? WHERE `user_id` = ? ", [
                            ...
                            if (result.affectedRows > 0) {
                                getUserDetailUserId(uObj.user_id, (_isDone, userObj) => {
    ...
    apiRouter.post('/api/profile_image', (req, res) => {
        ...
        form.parse(req, (err, _reqObj, files) => {
            if (err) {
                helper.ThrowHtmlError(err, res);
                return
            }
                        ...
                        if (err) {
                            ...
                            db.query("UPDATE `user_detail` SET `image` = ? WHERE `user_id` = ? ", [imageFileName, uObj.user_id], (err, result) => {
                                ...
                                if (result.affectedRows > 0) {
                                    ...
                                    getUserDetailUserId(uObj.user_id, (_isDone, uObj) => {
                                        res.json({ "status": "1", "payload": uObj })
                                    })
    ...
    apiRouter.post('/api/service_detail', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            db.query("SELECT `sd`.`service_name`, `sd`.`color`, `sd`.`icon`, `sd`.`top_icon` ,  `zwcs`.`zone_service_id`, `zwcs`.`service_provide`, `zwcs`.`status_message` FROM `zone_wise_cars_service` AS `zwcs` " +
                ...
                if (result.length > 0) {
                    var userStatus = "apiRouterroved";
                    switch (result[1][0].status) {
                        case 0:
                            userStatus = "No Verify"
                            break;
                        case 1:
                            userStatus = "Not apiRouterroved"
                            break
                        default:
                            break;
                    }
...
function checkAccessToken(helperObj, res, callback, requireType = "") {
    ...
    console.log(" test 1",helperObj.access_token)
    helper.CheckParameterValid(res, helperObj, ["access_token"], () => {
    console.log(" test 2",helperObj.access_token)
    ...
    console.log("ver result : ",result)
            ...
            if (result.length > 0) {
                console.log("VAMOS A DEBUGEAR 1")
                if (requireType != "") {
                    console.log("VAMOS A DEBUGEAR 2")
                    if (requireType == result[0].user_type) {
                        return callback(result[0])
                    } else {
                        console.log("VAMOS A DEBUGEAR 3")
                ...
                console.log("VAMOS A DEBUGEAR 4")
                console.log(" test 1",result)
                console.log("RequireType:", requireType);
               // console.log("DB user_type:", result[0].user_type);
...
// function checkAccessToken(helperObj, res, callback, requireType = "") {
//     helper.Dlog(helperObj.access_token);
//     console.log("test 1 (access_token):", helperObj.access_token);

//     helper.CheckParameterValid(res, helperObj, ["access_token"], () => {
//         const query = `
//             SELECT user_id, name, email, gender, mobile, mobile_code, auth_token, user_type, is_block, image, status
//             FROM user_detail
//             WHERE auth_token = ? AND (status = ? OR status = ?)
//         `;

//         db.query(query, [helperObj.access_token, "1", "2"], (err, result) => {
//             if (err) {
...
//                 console.log("VAMOS A DEBUGEAR 1");
//                 const user = result[0];
//                 console.log("User found:", user.user_id, "| Type:", user.user_type);
...
//                     } else {
//                         console.log("VAMOS A DEBUGEAR 3 - Tipo no coincide");
//                         return res.json({
//                             status: "0",
//                             code: "404",
...
//                 }
//             } else {
//                 console.log("VAMOS A DEBUGEAR 4 - Ningún usuario encontrado");
//                 console.log("Access token no coincide con ningún registro en la tabla `user_detail`.");
//                 return res.json({
//                     status: "0",
//                     code: "404",
